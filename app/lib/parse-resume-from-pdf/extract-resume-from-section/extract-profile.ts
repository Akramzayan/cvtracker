import { hasLetterAndAllUpperCase, isBold } from "../groupe-lines-into-sections";
...
const hasParenthesis=(item:TextItem) =>
/\([0-9]+\)/.test(item.text)
export const hasNumber = (item:TextItem) => 
/[0-9]/.test(item.text);
...
export const  hasComma=(item:TextItem) => 

item.text.includes(",");

//URL
export const matchUrl = (item:TextItem) => 
item.text.match(/\S+\.[a-z]+\/S+/);

const matchUrlHttp = (item:TextItem) => 
item.text.match(/https?:\/\/S+\.\S+/);

const matchUrlwww = (item:TextItem)=> 
item.text.match(/www\.\S+\.\S+/);

const hasSlash = (item:TextItem)=> 
item.text.includes("/");
