...
import { ResumeSectionToLines, line, lines,TextItem } from "./types";
...
const isSectionTtile =(line:line,lineNumber:number)=> {
    ...
    if(isBold(textItem) && hasLetterAndAllUpperCase(textItem)){
        return true
    }

    const text = textItem.text.trim()

    const textHasAlmost2words = 
    text.split(" ").filter((word)=> word !=="&").length<=2;

    const startsWithCapitalLetter = /[A-Z]/.test(text[0])

    if(textHasAlmost2words && hasOnlyLettersSpacesAmpersands(textItem)  && startsWithCapitalLetter &&SECTION_TITLE_KEYWORDS.some((keyword) => text.toLowerCase().includes(keyword))){
        return true
    }
    return false 
...
export const hasLetterAndAllUpperCase =(item:TextItem) => 
hasLetter(item) && 
